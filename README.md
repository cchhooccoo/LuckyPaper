...
数据来源是 500彩票网(500.com) 的历史开奖数据页面，这是社区里长期被验证可用、对自动化请求比较宽容的数据源
（相比之下，官方接口 cwl.gov.cn / sporttery.cn 会直接拦截 GitHub Actions 这类海外机房IP，返回 403，
所以脚本没有走官方接口）。500.com 毕竟也是第三方网站，它随时可能调整页面结构，一旦失效排查思路：

1. 看 Actions 运行日志里 `[ERROR]` 后面打印的原始响应片段，判断是"完全请求不通"还是
   "请求通了但表格结构变了"
2. 如果是表格结构变了：把 GitHub Actions 日志里报错附近的原始内容片段贴给我，我可以帮你
